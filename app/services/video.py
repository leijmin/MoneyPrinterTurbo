...
def fix_video_for_moviepy(video_path: str, force_h264: bool = True) -> str:
    ...
    # 获取视频旋转信息
    rotation = get_video_rotation(video_path)
    logger.info(f"视频尺寸: {width}x{height}, 旋转角度: {rotation}°")
    ...
    if "hevc" in codec.lower() or "h265" in codec.lower():
        ...
        logger.info("HEVC/H265编码，需要转换为H264")
    
    # 有旋转角度需要处理
    if rotation in [90, 270, -90, 180]:
        needs_processing = True
        logger.info(f"视频有旋转角度({rotation}°)，需要处理")
    
    # 如果不需要处理，直接返回原文件
    if not needs_processing and not force_h264:
        logger.info("视频不需要处理，使用原始文件")
        return video_path
    ...
    # 构建ffmpeg命令，确保编码兼容且保持原始宽高比
    cmd = [
        "ffmpeg", "-y",
        "-i", video_path,
        "-map_metadata", "-1",  # 移除全部元数据
        "-vf", f"{rotate_filter}format=yuv420p",  # 只旋转和转换像素格式，不缩放
    ...
    try:
        if check_result.returncode == 0:
            ...
            if data.get("streams"):
                new_width = int(data["streams"][0].get("width", 0))
                new_height = int(data["streams"][0].get("height", 0))
                logger.info(f"处理后视频尺寸: {new_width}x{new_height}")
    except Exception as e:
        logger.error(f"获取处理后视频尺寸失败: {e}")
...
def generate_video_ffmpeg(
    ...
    try:
        # 获取视频信息以确定最佳的处理方式
        video_info_cmd = ["ffprobe", "-v", "error", "-select_streams", "v:0", 
        ...
        try:
            ...
            if info_result.returncode == 0:
                ...
                if data.get("streams"):
                    ...
                    video_info = {"width": v_width, "height": v_height, "codec": codec_name}
        except Exception as e:
            logger.error(f"获取视频信息失败: {e}")
            
        # 获取视频旋转信息
        rotation = get_video_rotation(video_path)
        logger.info(f"视频原始尺寸: {video_info.get('width', 0)}x{video_info.get('height', 0)}, 旋转: {rotation}°, 编码: {video_info.get('codec', '')}")
        ...
        # HEVC编码需要转码
        if video_info.get('codec', '').lower() in ['hevc', 'h265']:
            needs_processing = True
            logger.info("HEVC/H265编码，需要转换为H264")
        ...
        if not needs_processing:
            ...
            video_cmd = [
                "ffmpeg", "-y",
                "-i", video_path,
                "-vf", f"{rotate_filter}format=yuv420p",  # 只处理旋转和像素格式，不缩放
                "-c:v", "libx264",
                "-preset", "medium",
                "-crf", "23",
                "-an",  # 不包含音频
                    ...
                    if "streams" in data and data["streams"]:
                        ...
                        if "hevc" in codec.lower() or "h265" in codec.lower() or width > 1920 or height > 1920:
                            ...
                            transcode_cmd = [
                                ...
                                "-crf", "23",
                                "-preset", "fast",
                                "-vf", "scale='min(1920,iw)':'min(1080,ih)',format=yuv420p",
                                "-c:a", "aac",
                                "-b:a", "128k",
                                "-map_metadata", "-1",  # 移除所有元数据
                                output_path
